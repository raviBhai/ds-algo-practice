...
public class RainTrappedWater {
    public static void main(String[] args) {
        RainTrappedWater rainTrappedWater = new RainTrappedWater();
    ...
    public int trap3(final List<Integer> A) {
        ...
        for (int j = size - 2; j >= 0; j--) {
            right[j] = Math.max(right[j + 1], A.get(j));
        }

        for (int k = 0; k < size; k++) {
            water = water + Math.min(left[k], right[k]) - A.get(k);
        }
