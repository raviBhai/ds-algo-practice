...
We can get this using both BFS and DFS

BFS : similar to that used in _10_BottomViewOfBinaryTree

In BFS, if we move from left to right, we have to override in map every time we get the same level.
However, if we move from right to left, we will not override in the map.

DFS :
We can traverse root, right, left
we can pass a variable level.


 */
public class _11_RightViewOfBinaryTree {
